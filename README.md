...
3. Ejecutar el esquema: pegar en el SQL Editor de Supabase, en orden, los archivos de
   `supabase/migrations/` (`0001_init.sql` … `0005_precio_canchas.sql`).
...
- `/disponibilidad` — turnos por club/sede/cancha/fecha (con precio); un jugador logueado puede **reservar** un turno libre y **cancelar** los suyos
- `/admin` — panel del admin del club: gestiona sedes y canchas (incl. precio) y ve/cancela las reservas de su club (requiere sesión de admin)
